...
namespace mad::core {
    ...
    std::unique_ptr<LevelRunner> LevelLoaderFromServer::load(std::shared_ptr<EventDispatcher> global_dispatcher,
        ...
        camera->turn_on(*level_dispatcher, keys[LevelLoaderFromServer::IdKeys::Hero], camera_smoothness,
    ...
    std::unordered_map<LevelLoaderFromServer::IdKeys, Entity::Id> LevelLoaderFromServer::create_world(std::shared_ptr<LocalWorld> world) {
        std::stringstream m_level_map_stream;
        m_level_map_stream << m_level_map;
        ...
        while (std::getline(m_level_map_stream, map_line)) {
            for (char object : map_line) {
                switch (m_objects[object]) {
                    ...
                    case Objects::Hero: {
                        ...
                        r_hero_id = keys[LevelLoaderFromServer::IdKeys::Hero] = hero_id;
                        break;
                    }
                    case Objects::Enemy1: {
                        break;
    ...
    void LevelLoaderFromServer::create_block(std::shared_ptr<LocalWorld> world, Vec2d position,
    ...
    void LevelLoaderFromServer::create_decoration(std::shared_ptr<LocalWorld> world, Vec2d position, Objects object) {
    ...
    void LevelLoaderFromServer::create_mob(std::shared_ptr<LocalWorld> world, Vec2d position) {
        std::filesystem::path source(m_config_json["animated_resources"]);
