...
void mad::core::MobController::handle(const mad::core::Event &event) {
    if (event.type == mad::core::Event::Type::KeyPressed) {
        ...
        if (keystroke.key_id == sf::Keyboard::Key::Q) {
            for (Entity::Id entity_id : m_storage->extract(TagFilter(Entity::Type::Hero))) {
                if (&m_storage->get_entity(entity_id) != nullptr && cast_to_or_null<Mob>(m_storage->get_entity(entity_id)) != nullptr) {
                    ...
                    if(m_storage->extract(TagRadiusFilter(hero->get_position(), 200.0f, Entity::Type::Enemy)).size() > 0){
                        auto mob = cast_to_or_null<Mob>(m_storage->get_entity(m_storage->extract(TagRadiusFilter(hero->get_position(), 200.0f, Entity::Type::Enemy))[0]));
...
void mad::core::MobController::control() {
    for (Entity::Id entity_id : m_storage->extract(TagFilter(Entity::Type::Enemy))) {
        if (&m_storage->get_entity(entity_id) != nullptr && cast_to_or_null<Mob>(m_storage->get_entity(entity_id)) != nullptr) {
            ...
            if(m_storage->extract(TagRadiusFilter(mob->get_position(), 100.0f, Entity::Type::Hero)).size() > 0){
                auto hero = cast_to_or_null<Mob>(m_storage->get_entity(m_storage->extract(TagRadiusFilter(mob->get_position(), 100.0f, Entity::Type::Hero))[0]));
